...
[Thread Members Update](#DOCS_TOPICS_GATEWAY/thread-members-update) will only include if other users joined or left a thread if you have the [Request Guild Members](#DOCS_TOPICS_GATEWAY/request-guild-members) intent.
...
| [Thread Member Update](#DOCS_TOPICS_GATEWAY/thread-member-update)                   | you joined or were added to a thread                                                                                             |
| [Thread Members Update](#DOCS_TOPICS_GATEWAY/thread-members-update)                 | some user(s) joined or left a thread                                                                                             |
...
Sent when a thread is created, relevant to the current user, or when the current user is added to a private thread. The inner payload is a [channel](#DOCS_RESOURCES_CHANNEL/channel-object) object.  When being added to an existing private thread, includes a [thread member](#DOCS_RESOURCES_CHANNEL/thread-member) object.
...
| channel_ids? | array of snowflake                                                     | the parent channel ids whose threads are being synced.  If omitted, then threads were synced for the entier guild |
...
| members      | array of [thread member](#DOCS_RESOURCES_CHANNEL/thread-member-object) | any thread member objects from the synced threads for the current user, indicating which threads are joined       |

#### Thread Member Update

Sent when the current user is added to a thread.  The inner payload is a [thread member](#DOCS_RESOURCES_CHANNEL/thread-member-object) object.

#### Thread Members Update

Sent when anyone is added to or removed from a thread.  If the current user does not have the [Request Guild Members](#DOCS_TOPICS_GATEWAY/request-guild-members) intent, then this event will only be sent if the current_user was added or removed from the thread.
...
| added_members?       | array of [thread member](#DOCS_RESOURCES_CHANNEL/thread-member-object) | the users who were added to the thread                          |
| removed_member_ids?  | array of snowflake                                                     | the id of the users who were removed from the thread            |
